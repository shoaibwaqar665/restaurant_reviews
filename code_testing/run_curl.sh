...
URL="$1"
# Extract the slug from the URL to use as filename
# Extract slug from URL, remove query params, and replace unsafe characters
...
while true; do
  ...
  if [ "$HTTP_STATUS" -eq 200 ]; then
    ...
    echo "Failed with status $HTTP_STATUS, retrying in 5 seconds..."
    sleep 5
  fi
done
